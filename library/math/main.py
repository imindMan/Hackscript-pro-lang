...
def pow(value):
    if not isinstance(value[0].value, int) or isinstance(value[0].value, float) or isinstance(value[1].value, int) or isinstance(value[1].value, float):
        return "<code> err: InvalidObject/Somehow the objects aren't numbers"
    return value[0].value ** value[1].value


def absolute(value):
    if not isinstance(value[0].value, int) or isinstance(value[0].value, float):
